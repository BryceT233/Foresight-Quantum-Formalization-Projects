...
def render_expansion(stem, degree, terms, common):
    """A standalone `bch*Term = <explicit monomial chain>` lemma.

    Proving this in its own (small) goal is what keeps the cancellation identities inside the
    default heartbeat budget: the expensive `simp only` expansion then runs with few subterms
    instead of on the whole identity.
    """
    chain = []
    for word, (n, d) in terms:
        num = n * (common // d)
        letters = " * ".join("a" if x else "b" for x in word)
        chain.append("(%s / %d : ℚ) • (%s)" % (num, common, letters))
    body, cur = [], "      "
    for item in chain:
        piece = ("" if cur.strip() == "" else " + ") + item
    ...
    body.append(cur)
    return "\n".join([
        "/-- `%s` as an explicit monomial chain, for the cancellation identities: proving the" % stem,
        ...
        "private lemma %s_expand {𝔸 : Type*} [NormedRing 𝔸] [NormedAlgebra ℚ 𝔸] (a b : 𝔸) :"
            % stem,
        "    %s a b =" % stem,
    ] + body + [
        "  := by",
        "  unfold %s" % stem,
        "  simp only [one_smul, %sCoeffs, %sWords, List.ofFn_succ, List.ofFn_zero,"
            % (stem, stem),
...
def main():
    ...
    ap.add_argument("--expand", metavar="STEM", help="emit the expansion lemma for one term")
    ...
    if args.expand:
        match = [t for t in TERMS if t[0] == args.expand]
        if not match:
            raise SystemExit("unknown term %s" % args.expand)
        stem, _, deg = match[0]
        print(render_expansion(stem, deg, parsed[stem], commons[stem]))
        return 0
