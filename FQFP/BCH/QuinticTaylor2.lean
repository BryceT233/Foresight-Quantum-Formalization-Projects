...
Hand-ported from `Lean-BCH/BCH/Basic.lean:3028-4830`. The source writes the same words as explicit
`+`-chains of 75 and 105 monomials in `{x, V, y}` and then bounds the remainder one monomial at a
time (about 1200 lines, including 2V/3V/4V lemmas that nothing outside `Basic.lean` uses). Here the
words are data and each definition is a `∑` over `wordProdList`, which the arity-free norm API
bounds in a single application.
