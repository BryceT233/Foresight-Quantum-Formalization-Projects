...
(`BCHTerms.bchQuinticGroup*Words`), so `WordExpansion.norm_sum_binWord_diff_le` produces the whole
...
word-layer API (`WordExpansion.norm_binWord_le` and hence `norm_prod_sub_prod_le`), which is stated
...
theorem norm_bchQuinticGroup1_diff_le (z x y : 𝔸) :
  ...
  have hcard : (∑ i : Fin 4, ((Finset.univ.filter fun j => bchQuinticGroup1Words i j = 0).card : ℝ))
      = 10 := by
    rw [← Nat.cast_sum, card_sum_bchQuinticGroup1Words]
    norm_num
  have hM0 : (0 : ℝ) ≤ ‖z‖ + ‖x‖ + ‖y‖ := by positivity
  have hz : ‖z‖ ≤ ‖z‖ + ‖x‖ + ‖y‖ := by linarith [norm_nonneg x, norm_nonneg y]
  have hx : ‖x‖ ≤ ‖z‖ + ‖x‖ + ‖y‖ := by linarith [norm_nonneg z, norm_nonneg y]
  have hy : ‖y‖ ≤ ‖z‖ + ‖x‖ + ‖y‖ := by linarith [norm_nonneg z, norm_nonneg x]
  have key : ‖∑ i, ((List.ofFn (bchQuinticGroup1Words i)).map ![z, y]).prod
        - ∑ i, ((List.ofFn (bchQuinticGroup1Words i)).map ![x, y]).prod‖
      ≤ (∑ i, ((Finset.univ.filter fun j => bchQuinticGroup1Words i j = 0).card : ℝ))
          * (‖z‖ + ‖x‖ + ‖y‖) ^ 4 * ‖z - x‖ :=
    norm_sum_binWord_diff_le (v := bchQuinticGroup1Words) (M := ‖z‖ + ‖x‖ + ‖y‖) z x y
      hz hx hy hM0
  unfold bchQuinticGroup1
  exact key.trans_eq (by rw [hcard])
...
theorem norm_bchQuinticGroup4_diff_le (z x y : 𝔸) :
  ...
  have hcard : (∑ i : Fin 10,
      ((Finset.univ.filter fun j => bchQuinticGroup4Words i j = 0).card : ℝ)) = 25 := by
  ...
  have hx : ‖x‖ ≤ ‖z‖ + ‖x‖ + ‖y‖ := by linarith [norm_nonneg z, norm_nonneg y]
  have hy : ‖y‖ ≤ ‖z‖ + ‖x‖ + ‖y‖ := by linarith [norm_nonneg z, norm_nonneg x]
  have key : ‖∑ i, ((List.ofFn (bchQuinticGroup4Words i)).map ![z, y]).prod
        - ∑ i, ((List.ofFn (bchQuinticGroup4Words i)).map ![x, y]).prod‖
      ≤ (∑ i, ((Finset.univ.filter fun j => bchQuinticGroup4Words i j = 0).card : ℝ))
          * (‖z‖ + ‖x‖ + ‖y‖) ^ 4 * ‖z - x‖ :=
    norm_sum_binWord_diff_le (v := bchQuinticGroup4Words) (M := ‖z‖ + ‖x‖ + ‖y‖) z x y
      hz hx hy hM0
...
theorem norm_bchQuinticGroup6_diff_le (z x y : 𝔸) :
  ...
  have hcard : (∑ i : Fin 14,
      ((Finset.univ.filter fun j => bchQuinticGroup6Words i j = 0).card : ℝ)) = 35 := by
  ...
  have hx : ‖x‖ ≤ ‖z‖ + ‖x‖ + ‖y‖ := by linarith [norm_nonneg z, norm_nonneg y]
  have hy : ‖y‖ ≤ ‖z‖ + ‖x‖ + ‖y‖ := by linarith [norm_nonneg z, norm_nonneg x]
  have key : ‖∑ i, ((List.ofFn (bchQuinticGroup6Words i)).map ![z, y]).prod
        - ∑ i, ((List.ofFn (bchQuinticGroup6Words i)).map ![x, y]).prod‖
      ≤ (∑ i, ((Finset.univ.filter fun j => bchQuinticGroup6Words i j = 0).card : ℝ))
          * (‖z‖ + ‖x‖ + ‖y‖) ^ 4 * ‖z - x‖ :=
    norm_sum_binWord_diff_le (v := bchQuinticGroup6Words) (M := ‖z‖ + ‖x‖ + ‖y‖) z x y
      hz hx hy hM0
...
theorem norm_bchQuinticGroup24_diff_le (z x y : 𝔸) :
  ...
  have hcard : (∑ i : Fin 2,
      ((Finset.univ.filter fun j => bchQuinticGroup24Words i j = 0).card : ℝ)) = 5 := by
  ...
  have hx : ‖x‖ ≤ ‖z‖ + ‖x‖ + ‖y‖ := by linarith [norm_nonneg z, norm_nonneg y]
  have hy : ‖y‖ ≤ ‖z‖ + ‖x‖ + ‖y‖ := by linarith [norm_nonneg z, norm_nonneg x]
  have key : ‖∑ i, ((List.ofFn (bchQuinticGroup24Words i)).map ![z, y]).prod
        - ∑ i, ((List.ofFn (bchQuinticGroup24Words i)).map ![x, y]).prod‖
      ≤ (∑ i, ((Finset.univ.filter fun j => bchQuinticGroup24Words i j = 0).card : ℝ))
          * (‖z‖ + ‖x‖ + ‖y‖) ^ 4 * ‖z - x‖ :=
    norm_sum_binWord_diff_le (v := bchQuinticGroup24Words) (M := ‖z‖ + ‖x‖ + ‖y‖) z x y
      hz hx hy hM0
